...
class SAP_HuggingFace_AI {
    ...
    public function generate_image($prompt, $options = array()) {
        ...
        if ($status_code === 503) {
            ...
            if (isset($data['error']) && strpos($data['error'], 'loading') !== false) {
                return new WP_Error('model_loading', __('Model is loading. Please wait a moment and try again.', 'seo-analytics-pro'));
            }
        }

        if ($status_code !== 200) {
            $data = json_decode($response_body, true);
            $error_message = $data['error'] ?? __('Unknown error occurred', 'seo-analytics-pro');
    ...
    public function test_connection() {
        ...
        return array(
            'status' => 'success',
            'message' => __('Hugging Face API connection successful', 'seo-analytics-pro'),
